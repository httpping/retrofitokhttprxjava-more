 ...
 * 管理所有的接口列表
 */
public class SearchDataManager extends BasicDataManager{


    private static SearchDataManager dataManager ;
    ...
    public synchronized static SearchDataManager newInstance() {
        if (dataManager == null){
            dataManager = new SearchDataManager();
            dataManager.init(Contents.url_rr_search);
        }
        return  dataManager;
    }
...
//                .addConverterFactory(GsonConverterFactory.create())
                ...
                .build();
        vpNewtWork = new VpHttpClient.Builder().addBaseUrl(baseUrl).addRetrofit(mRetrofit).build();
    ...
    public static XsbSerarchServ SearchServ(){
        return newInstance().getInterIml(XsbSerarchServ.class);
    }
